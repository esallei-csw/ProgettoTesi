...
begin
  Result := PopulateCell<TCalendarModel>(Format(QUERY_CALENDAR,[IntToStr(ACellId)]), QueryUtilityHandler.QueryToCalendar);
end;
...
begin
  ...
  try
    ...
    //Result.Partials := PopulateCell<TPartialModel>(Format(QUERY_PARTIALS, [intToStr(ACellId), FloatToStr(Result.MaintenanceData[0].LastMaintenance)]), QueryUtilityHandler.QueryToPartialsList);
